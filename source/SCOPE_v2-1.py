...
if selection >= len(available_obs):
    ...
    print("\033[93m"
          + "Warning: Location unavailable, selected the last one instead."
          + "\033[0m")
...
dec_lower = (min_alt + obs_loc.lat - 90*u.deg).wrap_at(360*u.deg)
...
write_output = input("\033[32m"
                     ...
                     + "\033[0m").upper() in YES
if write_output:
    with open("output/{}_{}.cfg".format(obs_date, obs_short),
              "w+") as file:
        ...
        file.write("ST_END: ")
        file.write((st + obs_sky_rotation).wrap_at(24*u.h).to_string(unit=u.hour) + "\n")
